import { $, show, hide, copyToClipboard, ready } from "/assets/helpers.js";

const dom = {
	badgeList: $("badge-list"),
	addBadgeBtn: $("add-badge"),
	badgeOutput: $("badge-output"),
...
function updateOutput() {
	...
	dom.badgeOutput.textContent = badges.length > 0 ? "> ## " + badges.join(" ") : "";
}
...
ready(() => {
	dom.addBadgeBtn.addEventListener("click", function () {
		addBadgeForm();
		updateOutput();
	});

	dom.copyBtn.addEventListener("click", function () {
		copyToClipboard(dom.badgeOutput.textContent, dom.copySuccess);
		dom.copySuccess.classList.add("show");
		setTimeout(() => dom.copySuccess.classList.remove("show"), 2000);
	});
